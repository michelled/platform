...
<form class="stack" method="POST" action="{{ localized_route('register-languages') }}" novalidate>
    ...
    <fieldset class="stack">
        ...
        <p>{{ __('Please choose the language or languages you would like to use on this website.
Later, you will have the chance to choose the language or languages for your consultations.') }}</p>
        ...
        <div class="field @error('signed_language') field--error @enderror stack">
            ...
            <x-hearth-select name="signed_language" :options="Spatie\LaravelOptions\Options::forArray(['ase' => __('American Sign Language (ASL)'), 'fcs' => 'Langue des signes québécoise (LSQ)'])->nullable(__('Choose a language…'))->toArray()" :selected="old('signed_language', '')" hinted />
